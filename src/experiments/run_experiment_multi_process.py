...
from .worker_multi_process_v2 import worker_run_batch_v2 as worker_run_batch, TrialConfig, CANNParamsNumeric
from ..models.cann import create_gaussian_kernel
from ..analysis.dog_fitting import fit_dog


def create_params(stp_type: str) -> CANNParamsNumeric:
    """Create CANN parameters based on STP type (Table 1)."""
    if stp_type == 'std':
        return CANNParamsNumeric(
            N=100,
            J0=0.13,
            a=0.5,
            tau=10.0,
...
def run_experiment_multi_process(
    ...
    if verbose:
        ...
        print(f"每个 Worker 的线程数: {threads_per_worker} (OMP)")
    ...
    worker_args = [
        (batch, kernel_np, params, trial_config, batch_size_per_worker, threads_per_worker)
