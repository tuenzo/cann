#!/usr/bin/env python3
"""
Reproduce Figure 2: Single-Layer CANN Experiments (Clean Version)
==================================================================
...
Usage:
    python scripts/run_fig2_clean.py [--output_dir results/fig2] [--quick]
    python scripts/run_fig2_clean.py [--batch_size 50]
"""
...
def main():
    parser = argparse.ArgumentParser(description='Reproduce Figure 2 (Clean Version)')
    ...
    parser.add_argument('--batch_size', type=int, default=50,
                        help='Batch size for jax.vmap (default: 50, 推荐 20-100)')
    ...
    if args.quick:
        ...
        args.batch_size = 5  # 快速测试用小 batch
    ...
    print(f"  总 Trials: {2 * args.n_runs * args.n_trials}")
    ...
    print(f"  - 预期加速: ~250x（相比原始 Python 循环）")
    print(f"  - 预期时间: ~{2*20*100/250/60:.1f} 分钟")
    ...
    print(f"  总 Trials: {2 * args.n_runs * args.n_trials}")
    print(f"  平均速度: {2*args.n_runs*args.n_trials/total_time:.1f} trials/秒")
    print(f"  输出目录: {output_dir}")
    ...
    if args.quick:
        ...
        estimated_full_time = full_trials / (2*args.n_runs*args.n_trials/total_time)
        print(f"\n  预计完整实验 (20 runs × 100 trials) 耗时: {estimated_full_time/60:.1f} 分钟")
        print("\n注意：这是快速测试模式。")
        print("完整实验请运行: python scripts/run_fig2_clean.py")
    else:
        print(f"\n🎉 完整实验完成！")
